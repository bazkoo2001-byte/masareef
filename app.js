        ...
        function getCurrentCurrencyCode() {
            return localStorage.getItem('currencyCode') || 'EGP';
        }
        ...
        window.setCurrency = function(code) {
            ...
            showAlert(`✅ تم تغيير العملة إلى ${CURRENCIES[code].name}`, 'success');
        };

        function renderCurrencyOptions() {
            const container = document.getElementById('currency-options-list');
            if (!container) return;
            const current = getCurrentCurrencyCode();
            container.innerHTML = Object.keys(CURRENCIES).map(code => {
                const c = CURRENCIES[code];
                const selected = code === current;
                return `
                    <button type="button" class="settings-menu-item" onclick="setCurrency('${code}')">
                        <div class="settings-menu-icon" style="background: linear-gradient(135deg,#0ea5e9,#0284c7); font-size:15px;">${c.symbol}</div>
                        <div class="settings-menu-text">
                            <div class="settings-menu-title">${c.name}</div>
                            <div class="settings-menu-sub">الرمز: ${c.symbol}</div>
                        </div>
                        <span class="settings-menu-chevron" style="color: ${selected ? 'var(--accent-success)' : 'transparent'}; font-size:18px;">✓</span>
                    </button>
                `;
            }).join('');
        }

        window.openCurrencySubpage = function() {
            openSubpage('💱 اختيار العملة', `
                <div class="settings-menu-card" id="currency-options-list"></div>
            `);
            renderCurrencyOptions();
        };
        ...
        function refreshAllCurrencyDisplays() {
            ...
            if (typeof updateSettlementsDashboard === 'function') updateSettlementsDashboard();
            if (typeof updatePotDashboard === 'function') updatePotDashboard();
            if (typeof updateExpensesLog === 'function') updateExpensesLog();
        }
        ...
        // ================================
        // خريطة الصفحات الفرعية اللي بترجع لصفحة "أب" معينة في التابس (مش تخرج من التطبيق)
        const SUBPAGE_PARENT = {
            'page-add': 'page-accounts',
            'page-accounts-dashboard': 'page-accounts',
            'page-settlement-view': 'page-settlement'
        };

        (function setupBackButtonHandling() {
            let overlayHistoryPushed = false;

            function isHubSubpageOpen() {
                const el = document.querySelector('.app-page.active');
                return !!(el && SUBPAGE_PARENT[el.id]);
            }
            ...
            function isAnyOverlayOpen() {
                ...
                if (isHubSubpageOpen()) return true;
                return false;
            }
            ...
            window.addEventListener('popstate', () => {
                ...
                if (modalOrSubpageOpen) {
                    ...
                    overlayHistoryPushed = false;
                } else if (isHubSubpageOpen()) {
                    const el = document.querySelector('.app-page.active');
                    switchPage(SUBPAGE_PARENT[el.id]);
                    overlayHistoryPushed = false;
                }
            });
        ...
        // زر الرجوع اليدوي من أي صفحة فرعية (إضافة / لوحة الحسابات / عرض التسوية) لصفحة التاب الأب
        window.goBackToHub = function() {
            const el = document.querySelector('.app-page.active');
            const parent = el && SUBPAGE_PARENT[el.id];
            if (parent) switchPage(parent);
                    ...
                    currentIds.forEach(id => {
                        if (!knownExpenseIds.has(id)) {
                            ...
                            if (exp.paidBy !== currentUser?.id) {
                                ...
                                showAlert(`💳 ${payerName} أضاف مصروف: ${exp.description || ''} (${parseFloat(exp.amount).toFixed(2)} ${getCurrencySymbol()})`, 'success');
                            }
            ...
            wrap.innerHTML = `<div class="bar-chart">${userIds.map(uid => {
                ...
                return `
                    <div class="bar-row">
                        ...
                        <div style="font-size:12px; font-weight:700; color:var(--text-secondary); min-width:60px; text-align:left;">${totals[uid].toFixed(0)} ${getCurrencySymbol()}</div>
                    </div>
                `;
            }).join('')}</div>`;
        }
        ...
        window.openPotSubpage = function() {
            openSubpage('🏺 الحصالة المشتركة', `
                <div class="pot-balance-box">
                    ...
                    <div class="pot-balance-amount" id="pot-balance-amount">0.00 <span>${getCurrencySymbol()}</span></div>
                </div>
        ...
        function updateParticipantsCheckboxes() {
            ...
            Object.keys(allUsers).forEach(userId => {
                ...
                currencySpan.textContent = getCurrencySymbol();
        ...
        function validateCustomSplit() {
            ...
            if (difference < 0.01) {
                ...
                validationDiv.innerHTML = `✅ المجموع: ${total.toFixed(2)} ${getCurrencySymbol()} = ${amount.toFixed(2)} ${getCurrencySymbol()}`;
                return true;
            } else if (total > amount) {
                validationDiv.style.background = '#f8d7da';
                validationDiv.style.color = '#721c24';
                validationDiv.style.border = '1px solid #f5c6cb';
                validationDiv.innerHTML = `⚠️ المجموع: ${total.toFixed(2)} ${getCurrencySymbol()} > ${amount.toFixed(2)} ${getCurrencySymbol()} (زيادة ${(total - amount).toFixed(2)} ${getCurrencySymbol()})`;
                return false;
            } else {
                validationDiv.style.background = '#fff3cd';
                validationDiv.style.color = '#856404';
                validationDiv.style.border = '1px solid #ffeaa7';
                validationDiv.innerHTML = `⚠️ المجموع: ${total.toFixed(2)} ${getCurrencySymbol()} < ${amount.toFixed(2)} ${getCurrencySymbol()} (نقص ${(amount - total).toFixed(2)} ${getCurrencySymbol()})`;
                return false;
            }
        }
        ...
        window.addExpense = async function(event) {
            ...
            if (fromPot) {
                ...
                if (amount > potBalance + 0.01) {
                    showAlert(`⚠️ رصيد الحصالة (${potBalance.toFixed(2)} ${getCurrencySymbol()}) مش كفاية لدفع ${amount.toFixed(2)} ${getCurrencySymbol()}`, 'warning');
                    return;
                }
                ...
                if (Math.abs(total - amount) > 0.01) {
                    showAlert(`⚠️ مجموع التقسيم (${total.toFixed(2)} ${getCurrencySymbol()}) لا يساوي المبلغ الكلي (${amount.toFixed(2)} ${getCurrencySymbol()})`, 'danger');
                    return;
                }
        ...
        function updateDashboard() {
            ...
            Object.keys(ledger).forEach(userId => {
                ...
                if (net > 0.01) {
                    statusText = `يطالب بـ: ${net.toFixed(2)} ${getCurrencySymbol()}`;
                    statusClass = 'positive';
                } else if (net < -0.01) {
                    statusText = `عليه دفع: ${Math.abs(net).toFixed(2)} ${getCurrencySymbol()}`;
                ...
                card.innerHTML = `
                    ...
                    <div class="summary-details">
                        <div class="summary-row">
                            ...
                            <span class="summary-value">${record.totalPaid.toFixed(2)} ${getCurrencySymbol()}</span>
                        </div>
                        <div class="summary-row">
                            <span class="summary-label">عليه:</span>
                            <span class="summary-value">${record.totalOwed.toFixed(2)} ${getCurrencySymbol()}</span>
                        </div>
                    </div>
        ...
        async function initBiometricUI() {
            const row = document.getElementById('biometric-security-row');
        ...
        function updatePotDashboard() {
            ...
            if (balanceEl) {
                balanceEl.innerHTML = `${computePotBalance().toFixed(2)} <span>${getCurrencySymbol()}</span>`;
            }
            ...
            listEl.innerHTML = contributions.map(c => {
                ...
                return `
                    <div class="pot-contribution-item">
                        ...
                        <span class="pot-contribution-amount">+${parseFloat(c.amount).toFixed(2)} ${getCurrencySymbol()}</span>
                    </div>
                `;
            }).join('');
        }
        ...
        window.submitContribution = async function(event) {
            ...
            try {
                ...
                showAlert(`تمت المساهمة بـ ${amount.toFixed(2)} ${getCurrencySymbol()} في الحصالة 🏺`, 'success');
            } catch (error) {
                showAlert('حدث خطأ: ' + error.message, 'danger');
            }
        };
        ...
        window.submitPayment = async function(event, mode, fromId, toId) {
            ...
            try {
                ...
                showAlert(`✅ سجّلنا إنك دفعت ${amount.toFixed(2)} ${getCurrencySymbol()}، مستني تأكيد الطرف التاني`, 'success');
            } catch (error) {
                showAlert('حدث خطأ: ' + error.message, 'danger');
            }
        };
        ...
        window.setSettlementMode = function(mode) {
            ...
            const titleEl = document.getElementById('settlement-view-title');
            if (titleEl) titleEl.textContent = mode === 'direct' ? '⚡ تسوية مباشرة' : '📋 تسوية تفصيلية';
            updateSettlementsDashboard();
        };

        window.openSettlementMode = function(mode) {
            setSettlementMode(mode);
            switchPage('page-settlement-view');
        };
            ...
            visibleSettlements.forEach((settlement, index) => {
                ...
                if (settlement.paidAmount > 0.01) {
                    ...
                    progressHtml = `
                        ...
                        <div class="settle-progress-label">اتدفع ${settlement.paidAmount.toFixed(2)} ${getCurrencySymbol()} من أصل ${settlement.grossAmount.toFixed(2)} ${getCurrencySymbol()}</div>
                    `;
                }
                ...
                if (currentUser?.id === settlement.toId && settlement.pendingPayments.length > 0) {
                    pendingHtml = settlement.pendingPayments.map(p => `
                        <div class="settle-pending-item">
                            <span>⏳ ${settlement.from} قال إنه دفعلك <strong>${parseFloat(p.amount).toFixed(2)} ${getCurrencySymbol()}</strong></span>
        ...
        function updateExpensesLog() {
            ...
            sortedUsers.forEach(userId => {
                ...
                header.innerHTML = `
                    ...
                    <div class="exp-group-total">${totalAmount.toFixed(2)} <span>${getCurrencySymbol()}</span></div>
                `;
                ...
                userExpenses.forEach(expense => {
                    ...
                    card.innerHTML = `
                        <div class="exp-card-top">
                            ...
                            <div class="exp-amount">${parseFloat(expense.amount).toFixed(2)}<span>${getCurrencySymbol()}</span></div>
                        </div>
        ...
        function updateStats() {
            ...
            document.getElementById('stat-total-expenses').textContent = totalExpenses.toFixed(2) + ' ' + getCurrencySymbol();
            document.getElementById('stat-my-paid').textContent = myPaid.toFixed(2) + ' ' + getCurrencySymbol();
            document.getElementById('stat-my-owed').textContent = myOwed.toFixed(2) + ' ' + getCurrencySymbol();
            document.getElementById('stat-my-net').textContent = myNet.toFixed(2) + ' ' + getCurrencySymbol();
        ...
        function renderReports() {
            ...
            statsBar.innerHTML = `
                ...
                <div class="reports-stat">
                    ...
                    <div class="reports-stat-label">إجمالي (${getCurrencySymbol()})</div>
                </div>
                <div class="reports-stat">
                ...
                function buildSettlementRowsHtml(list) {
                    ...
                    return list.map(s => `
                        <div class="report-settle-row">
                            ...
                            <div class="report-settle-amount">${parseFloat(s.amount).toFixed(2)} <span>${getCurrencySymbol()}</span></div>
                        </div>
                    `).join('');
                }
                        ...
                        <div style="flex:1; cursor:pointer; display:flex; align-items:center; gap:12px;" onclick="toggleReport('${report.id}')">
                            ...
                            <div>
                                ...
                                <div class="report-meta">
                                    ...
                                    <span class="report-meta-pill report-meta-amount">💰 ${parseFloat(report.totalExpenses || 0).toFixed(2)} ${getCurrencySymbol()}</span>
                                </div>
                            </div>
                        </div>
        ...
        window.shareSettlementSummary = async function() {
            ...
            const totalExpenses = Object.values(allExpenses).reduce((s, e) => s + (parseFloat(e.amount) || 0), 0);
            ctx.font = '600 16px Tahoma, Arial';
            ctx.fillText(`إجمالي مصاريف الدورة: ${totalExpenses.toFixed(2)} ${getCurrencySymbol()}`, width - 40, 122);
            ...
            if (settlements.length === 0) {
                ...
                settlements.forEach((s, i) => {
                    ...
                    ctx.fillText(`${s.amount.toFixed(2)} ${getCurrencySymbol()}`, 60, y + 42);
                    ctx.textAlign = 'right';
                });
            }
        ...
        window.switchPage = function(pageId) {
            ...
            // الصفحات الفرعية (إضافة/لوحة الحسابات/عرض التسوية) بتفتح من قائمة تاب أب، فلما تكون مفتوحة نفعّل نفس التاب في الشريط السفلي
            const navHighlightId = SUBPAGE_PARENT[pageId] || pageId;
        ...
        function restoreActivePage() {
            ...
            const validPages = ['page-home', 'page-add', 'page-accounts', 'page-accounts-dashboard', 'page-settlement', 'page-settlement-view', 'page-settings'];
