        ...
        window.toggleContactUs = function() {
            const panel = document.getElementById('contact-us-panel');
            const chevron = document.getElementById('contact-us-chevron');
            if (!panel) return;
            ...
            reports.forEach(report => {
                ...
                function buildSettlementRowsHtml(list) {
                    if (!list || list.length === 0) {
                        return '<div style="text-align:center;padding:12px;color:var(--color-success);">✅ كانت الحسابات متوازنة</div>';
                    }
                    return list.map((s, i) => `
                ...
                const directSettlementHtml = directSettlement !== undefined
                    ...
                    : '<div style="text-align:center;padding:12px;color:var(--text-muted);font-size:12px;">التقرير ده اتحفظ قبل إضافة التسوية المباشرة</div>';
