        ...
        // 🔐 الأمان والتشفير
        // ================================
        const ADMIN_PASSWORD_HASH = '5cbc023289bf7794bfecfd3a80c3f9e36e23c9d8a2766d764d2cf020e0426e81'; // 101093
        ...
        async function loadFirebaseConfigFromStorage() {
            try {
                ...
                initializeFirebaseWithConfig(configToUse);
        ...
        function applyUserRole() {
            ...
            // تحديث اسم المستخدم في صفحة الإعدادات
        ...
        window.addExpense = async function(event) {
            ...
            if (fromPot) {
                ...
                if (amount > potBalance + 0.01) {
                    showAlert(`⚠️ رصيد الكشة (${potBalance.toFixed(2)} ج.م) مش كفاية لدفع ${amount.toFixed(2)} ج.م`, 'warning');
                    return;
                }
                // مفيش تقسيم — المبلغ بيتخصم من الكشة مباشرة
            ...
            try {
                ...
                // مزامنة سجل الكشة المشتركة (منفصل عن المصاريف عشان يفضل موجود حتى بعد تصفير الدورة)
                if (fromPot) {
                    ...
                    // لو كان بيتدفع من الكشة قبل كده وتم إلغاء الخيار، رجّع سجل الكشة
        ...
        window.deleteExpense = async function(expenseId) {
            ...
            try {
                if (db) {
                    ...
                    // لو كان المصروف ده مدفوع من الكشة، رجّع المبلغ (احذف سجل الكشة المرتبط)
        ...
        function updatePotDashboard() {
            ...
            if (contributions.length === 0) {
                listEl.innerHTML = '<p class="exp-comments-empty">لسه محدش ساهم في الكشة</p>';
                return;
            }
        ...
        window.submitContribution = async function(event) {
            ...
            try {
                ...
                showAlert(`تمت المساهمة بـ ${amount.toFixed(2)} ج.م في الكشة 🏺`, 'success');
            } catch (error) {
                showAlert('حدث خطأ: ' + error.message, 'danger');
            }
        };
        ...
        function getConfirmedPaidAmount(fromId, toId) {
            const pairKey = getPairKey(fromId, toId);
            const payments = allSettlementPayments[pairKey] || {};
        ...
        function getPendingPayments(fromId, toId) {
            const pairKey = getPairKey(fromId, toId);
            const payments = allSettlementPayments[pairKey] || {};
        ...
        window.submitPayment = async function(event, fromId, toId) {
            event.preventDefault();
            const settleId = `${fromId}__${toId}`;
            ...
            const pairKey = getPairKey(fromId, toId);
            try {
                await push(ref(db, groupPath(`settlementPayments/${pairKey}`)), {
                    fromId: fromId,
                    toId: toId,
                    amount: amount,
                    status: 'pending',
        ...
        window.confirmPayment = async function(fromId, toId, paymentId) {
            const pairKey = getPairKey(fromId, toId);
            try {
                await set(ref(db, groupPath(`settlementPayments/${pairKey}/${paymentId}/status`)), 'confirmed');
                await set(ref(db, groupPath(`settlementPayments/${pairKey}/${paymentId}/confirmedAt`)), Date.now());
        ...
        window.rejectPayment = async function(fromId, toId, paymentId) {
            const pairKey = getPairKey(fromId, toId);
            try {
                await set(ref(db, groupPath(`settlementPayments/${pairKey}/${paymentId}`)), null);
        ...
        function computeVisibleSettlements() {
            ...
            // من المصاريف: كل مشارك عليه حصته للي دفع (ما عدا المصاريف المدفوعة من الكشة المشتركة)
            Object.keys(allExpenses).forEach(expenseId => {
                const expense = allExpenses[expenseId];
                if (expense.fromPot) return; // اتدفعت من الكشة، مفيش مديونية شخصية
                const amount = parseFloat(expense.amount) || 0;
                const payerId = expense.paidBy;
                const participants = expense.participants || [];
            ...
            settlements.forEach(s => {
                ...
                s.paidAmount = getConfirmedPaidAmount(s.fromId, s.toId);
                s.amount = Math.max(0, s.grossAmount - s.paidAmount);
                s.pendingPayments = getPendingPayments(s.fromId, s.toId);
            });
        ...
        function updateSettlementsDashboard() {
            ...
            const visibleSettlements = computeVisibleSettlements();
            ...
            visibleSettlements.forEach((settlement, index) => {
                ...
                const settleId = `${settlement.fromId}__${settlement.toId}`;
                ...
                if (currentUser?.id === settlement.fromId) {
                    payActionHtml = `
                        ...
                        <div class="settle-pay-panel" id="pay-panel-${settleId}" style="display:none;">
                            <form onsubmit="submitPayment(event, '${settlement.fromId}', '${settlement.toId}')">
                    ...
                    pendingHtml = settlement.pendingPayments.map(p => `
                        <div class="settle-pending-item">
                            ...
                            <div class="settle-pending-actions">
                                <button class="btn btn-success btn-sm" onclick="confirmPayment('${settlement.fromId}', '${settlement.toId}', '${p.id}')">✅ تأكيد الاستلام</button>
                                <button class="btn btn-outline btn-sm" onclick="rejectPayment('${settlement.fromId}', '${settlement.toId}', '${p.id}')">❌ لم يصلني</button>
                            </div>
                        </div>
                    `).join('');
                }
        ...
        function updateExpensesLog() {
            ...
            sortedUsers.forEach(userId => {
                ...
                // زر طي في آخر المجموعة (يظهر فقط لو فيها عمليات كتير)
                if (expensesCount >= 4) {
        ...
        function renderReports() {
            ...
            reports.forEach(report => {
                ...
                if (settlement.length === 0) {
                    settlementHtml = '<div style="text-align:center;padding:12px;color:var(--color-success);">✅ كانت الحسابات متوازنة</div>';
                } else {
                    settlement.forEach((s, i) => {
                        settlementHtml += `
                            <div class="report-settle-row">
                                <span class="report-settle-num">${i + 1}</span>
                ...
                card.innerHTML = `
                    <div class="report-header" onclick="toggleReport('${report.id}')">
                        <div>
                            ...
                            <div class="report-meta">${dateStr} • ${report.expensesCount || 0} عملية • ${parseFloat(report.totalExpenses || 0).toFixed(2)} ج.م</div>
                        </div>
                        <span class="report-chevron" id="report-chevron-${report.id}">◀</span>
                    </div>
                    <div class="report-body" id="report-body-${report.id}" style="display:none;">
                        <div class="report-section-title">💰 التسوية النهائية (من يدفع لمين):</div>
                        ${settlementHtml}
                    </div>
                `;
        ...
        window.confirmReset = async function(event) {
            ...
            if (passwordHash !== ADMIN_PASSWORD_HASH) {
                showAlert('رقم Admin غير صحيح!', 'danger');
                return;
            }
            
            // حساب التسوية الكاملة قبل التصفير
            ...
            // تصفير كل حاجة: المصاريف والسلف ودفعات التسوية (دورة جديدة نظيفة)
        ...
        window.shareSettlementSummary = async function() {
            if (!currentUser) return;
            const settlements = computeVisibleSettlements();
        ...
        function restoreActivePage() {
            ...
            const validPages = ['page-home', 'page-add', 'page-accounts', 'page-settlement', 'page-settings'];
